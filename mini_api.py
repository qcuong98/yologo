...
def use_model_1():
    _model = model.get_model("cfg/coco.data", "cfg/yolov3.cfg", "weights/yolov3.weights")
    return run_model(request.files, _model)
    
@app.route("/model_2", methods = ["POST"])
def use_model_2():
    _model = model.get_model("cfg/yologo.data", "cfg/yologo.cfg", "weights/yologo.weights")
    return run_model(request.files, _model)
